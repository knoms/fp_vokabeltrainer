...
public class Gui extends Application {
	...
	private Parent createSceneGraph() {
		...
		String dictionarySize = Integer.toString(TL.getDictionary().getSize())+  " Wˆrter im Wˆrterbuch vorhanden.";
		Label totalNumberOfVocabs= new Label(dictionarySize);
		Label UserTaskInfo= new Label("Bitte ‹bersetze folgendes Wort: "+ wordToTranslate);
		...
		Label userInfo = new Label("Wort zum Hinzuf√ºgen eingeben: ");
		...
		save.setOnAction(event-> {
			if(tf1.getText().length()>0&&tf2.getText().length()>0) {
				...
				userInfo.setText(a + " = "+ b + " wurde dem W√∂rterbuch hinzugef√ºgt");
				} catch(DictionaryException dE) {
					userInfo.setText("Hinzuf√ºgen fehlgeschlagen: "+ dE.getMessage());
				}
